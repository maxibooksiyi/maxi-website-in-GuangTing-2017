...
public class db2 {
	 ...
	 public String chaxun1()//这需要加个输入，知道你要查什么信息？包括行和列,给定主键和哪列，就可以输出对应的元素了对吧。需要输入两个元素。
	 {   String h = null;
		 try {
	        ...
	        try {
	            conn =
	               (Connection) DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/maxiweb?" +
...
//	                                   "\t" + dianhua+"\t"+money);
	 ...
	 public String jutishuchu(int a)//这需要加个输入，知道你要查什么信息？包括行和列,给定主键和哪列，就可以输出对应的元素了对吧。需要输入两个元素。
	 {   String h = null;
		 try {
	            // The newInstance() call is a work around for some
	            // broken Java implementations
